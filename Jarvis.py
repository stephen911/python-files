...
class ChatInterface(Frame):
    def __init__(self, master=None):
        ...
        help_option.add_command(label="About Jarvis", command=self.msg)
        ...
        ob = new_bot.greetMe()
        pr = "{}".format(str(ob)) + "\n"
        pr2 = "Jarvis : {}".format(str(ob)) + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, pr2)
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END)
        time.sleep(0)
        t2 = threading.Thread(target=self.speak, args=(pr,))
        t2.start()

    def save_chat(self):
        save = self.text_box.get("1.0", END)
        new_bot.save(save)
    ...
    def msg(self):
        tkinter.messagebox.showinfo("Jarvis v1.0",
                                    "Hello I am Jarvis. I am a digital assistant. My creator is Dr. Stephen Dapaah. "
         "He is a computer scientist. I am developed to make things easier, I can open browser and other applications. "
         "I can also play a particular song for you. I am trained to chat formally with users. I am created with Python and "
         "still under development.")

    def about(self):
        tkinter.messagebox.showinfo("Jarvis Developer",
                                    "Dr. Stephen Dapaah")
